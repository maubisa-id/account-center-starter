...
import { serviceUrl, type ServiceKey } from "@/lib/services";
...
export default async function UbahPaket() {
  ...
  const hasPlus = activeCodes.has("pro-plan");
  ...
  const items: BrowserItem[] = (await getCatalog())
    ...
    .map((i) => ({
      ...i,
      ctaUrl: (SCOPES as string[]).includes(i.scope) ? serviceUrls[i.scope as ServiceKey] : null,
    }));
  ...
  return (
    <div className="space-y-8">
      ...
      <SectionTitle
        ...
        title="Semua layanan Acme"
        desc="Langganan Pro, ikut kelas & sertifikasi, mulai bimbingan skripsi, atau jelajahi Book Universe. Setiap layanan berdiri sendiri, jadi menambah satu layanan tidak mengubah langganan lain yang sedang aktif. Pilih kategori untuk memfilter."
      />

      <CatalogBrowser items={items} activeCodes={[...activeCodes]} hasPlus={hasPlus} />

      <p className="text-xs leading-relaxed text-zinc-400">
        Layanan berlangganan &amp; sekali beli dibayar dengan aman lewat Midtrans. Layanan
        bimbingan skripsi dimulai dari halaman layanannya. Item bertanda &quot;Segera hadir&quot;
        akan aktif begitu tersedia.
      </p>
    </div>
  );
}
