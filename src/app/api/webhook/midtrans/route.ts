...
export async function POST(req: Request) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (!invoice) {
        ...
        // RECURRING (Pro Midtrans-managed): tagihan ulang otomatis datang dengan
      ...
      if (status === "paid") {
        ...
        // (kelas_acme) yang mengaktifkan setelah lisensi siap. Produk lain langsung aktif.
    ...
    console.error(`[webhook] gagal proses order=${orderId}:`, e instanceof Error ? e.message : "unknown");
  ...
  if (out.receipt) {
    try {
      ...
      console.error(
        `[webhook] gagal kirim struk order=${orderId}:`,
  ...
  // RECURRING (Pro): setelah charge pertama kartu LUNAS, daftarkan langganan ke
