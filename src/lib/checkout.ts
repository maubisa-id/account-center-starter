...
//  - input { event } -> acara berbayar (Events), eventCode di-snapshot.
...
export async function resolveCheckout(input: CheckoutInput): Promise<ResolvedCheckout | CheckoutError> {
  // ── ACARA BERBAYAR (Events & sejenis) ────────────────────────────────
  // Harga & judul OTORITATIF diambil dari baris acara di Directus (bukan harga
  // produk events yang tetap), sesuai katalog-produk.md: "ambil judul + harga
  ...
  if (input.event) {
    const item = CATALOG.find((c) => c.key === "events"); // template scope/itemType 'event'
    ...
    if (!pricing) {
      ...
      // Dev tanpa Directus: pakai contoh katalog events.
