...
// Key di user_preferences: "interest:<slug>" dan "goal:<slug>". Lihat directus-acme/docs/acara-pendaftaran.md.
...
export function waktuRelatif(d?: Date | null): string {
  if (!d) return "-";
...
// Ubah slug jadi label manusiawi: "free-events-produktif" -> "Free Events Produktif".
// Akronim tertentu di-uppercase. Dipakai sbg fallback terakhir judul entitlement/acara.
const ACRONYMS = new Set(["acme", "ai", "id", "pdf", "hr", "ux", "ui"]);
...
// Subjudul entitlement: "{jenis} · {nama produk / layanan}", mis. "Langganan · Pro".
