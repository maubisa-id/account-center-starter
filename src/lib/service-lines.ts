// Sumber tunggal nama LINI LAYANAN Acme (level scope). Dipisah dari lib/services.ts
// supaya aman diimpor komponen klien (murni data, tanpa env/entitlement). Nama diambil dari
// halaman publik example.com/layanan + business plan Acme, jadi Account Center selaras dengan
// web utama.
//
// Dua level yang HARUS jelas bedanya di UI:
//  - LINI (scope)  = Acme Lulus / Berkembang / Mahir / Book Universe  → yang "dibuka" (SSO)
//  - PRODUK (item) = Pro, Events, Kelas X, Bimbingan Skripsi …       → yang "dibeli"
...
// Urutan kanonik = perjalanan Acme (Lulus → Berkembang → Mahir → Book). Dipakai konsisten di
...
export const SERVICE_LINE: Record<ServiceScope, { name: string; tagline: string }> = {
  thesis: { name: "Acme Lulus", tagline: "Bimbingan skripsi & tugas akhir" },
  app: { name: "Acme Berkembang", tagline: "Pro · pengembangan diri & komunitas" },
  kelas: { name: "Acme Mahir", tagline: "Kelas & sertifikasi profesional" },
