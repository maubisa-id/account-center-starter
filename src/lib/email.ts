...
export async function sendEmail({ to, subject, html, text, from, replyTo }: SendArgs): Promise<{ ok: boolean; dev: boolean; error?: string }> {
  ...
  // MODE DEMO: SELALU tangkap ke kotak masuk demo (in-memory) supaya pengunjung bisa MELIHAT
  // kode/isi email langsung di /demo/kotak — tanpa perlu inbox sungguhan. Berlaku baik SMTP diset
  // maupun tidak, jadi alur verifikasi (OTP daftar) tetap bisa diselesaikan siapa pun yang mencoba
  // demo. Di produksi (NEXT_PUBLIC_DEMO_MODE != 1) blok ini mati total → tak ada email mengendap.
  if (demoMailboxEnabled()) captureDemoEmail({ to, subject, html, text: text ?? stripHtml(html) });
