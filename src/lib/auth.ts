...
// Better Auth mengelola tabel auth_* sendiri. Identitas billing (acme_core.users)
...
// SSO lintas subdomain hanya diaktifkan di produksi (*.example.com), bukan localhost.
const COOKIE_DOMAIN = AUTH_URL.includes("example.com") ? ".example.com" : undefined;
// Origin yang dipercaya untuk SSO: semua subdomain Acme boleh memanggil auth API
// (mis. app/kelas membaca sesi/preferensi). Di lokal cukup localhost.
const TRUSTED_ORIGINS = COOKIE_DOMAIN
  ? [
      "https://example.com",
      "https://akun.example.com",
      "https://app.example.com",
      "https://kelas.example.com",
...
export const auth = betterAuth({
  ...
  // Cookie sesi berlaku untuk seluruh *.example.com supaya "Buka layanan" auto-login
  ...
  plugins: [
    ...
    emailOTP({
      ...
      async sendVerificationOTP({ email, otp, type }) {
        // Kirim kode via email branded (no-reply@example.com). Fallback: cetak ke konsol.
  ...
  databaseHooks: {
    user: {
      create: {
        after: async (user) => {
          // Buat/tautkan core user (acme_core) berdasar email (dikunci UNIQUE).
