...
// Cek host tepat, BUKAN substring. `AUTH_URL.includes("maubisa.id")` bisa disalip host jahat
// seperti "maubisa.id.evil.com" atau "evil-maubisa.id" (CodeQL js/incomplete-url-substring-
// sanitization). Parse URL lalu cocokkan hostname persis / subdomain sah.
function isMaubisaHost(rawUrl: string): boolean {
  try {
    const host = new URL(rawUrl).hostname.toLowerCase();
    return host === "maubisa.id" || host.endsWith(".maubisa.id");
  } catch {
    return false;
  }
}
// Domain cookie SSO. Default: `.maubisa.id` bila AUTH_URL di bawah maubisa.id (login sekali
// kebawa ke semua subdomain). BISA di-override lewat AUTH_COOKIE_DOMAIN:
//   - AUTH_COOKIE_DOMAIN="host"  -> cookie HOST-ONLY (tak dibagikan lintas subdomain).
//     WAJIB untuk DEMO di demo.akun.maubisa.id supaya sesinya TIDAK bertabrakan dengan
//     produksi akun.maubisa.id (nama cookie sama + domain .maubisa.id = saling menimpa).
//   - AUTH_COOKIE_DOMAIN=".contoh.id" -> pakai domain itu (fork/merek lain).
const COOKIE_DOMAIN = (() => {
  const raw = process.env.AUTH_COOKIE_DOMAIN;
  if (raw !== undefined && raw !== "") {
    return raw.toLowerCase() === "host" ? undefined : raw;
  }
  return isMaubisaHost(AUTH_URL) ? ".maubisa.id" : undefined;
})();

// Origin tepercaya untuk auth API. SELALU sertakan origin app SENDIRI (AUTH_URL) supaya
// instance apa pun (mis. demo.akun.maubisa.id) tak menolak permintaannya sendiri. Bila SSO
// domain-wide `.maubisa.id` aktif, tambah subdomain resmi (`bayar.` untuk checkout tamu, §4).
const ownOrigin = (() => {
  try {
...
const SSO_ORIGINS =
  COOKIE_DOMAIN === ".maubisa.id"
    ? [
        "https://maubisa.id",
        "https://akun.maubisa.id",
        "https://bayar.maubisa.id",
...
const TRUSTED_ORIGINS = Array.from(
  new Set([ownOrigin, ...SSO_ORIGINS, "http://localhost:3000"].filter(Boolean)),
) as string[];
...
// PENGECUALIAN DEMO: build demo (NEXT_PUBLIC_DEMO_MODE=1) sengaja TANPA captcha — ini demo
// publik dengan data contoh, bukan akun asli, dan widget Turnstile eksternal sering diblokir
// ad-blocker/privacy sehingga menghalangi pengunjung mencoba. Produksi tak pernah set flag ini.
...
const captchaPlugins = TURNSTILE_SECRET
  ? [
      captcha({
        ...
        endpoints: ["/sign-up/email", "/sign-in/email", "/forget-password/email-otp"],
      }),
    ]
  : [];
...
export const auth = betterAuth({
  ...
  databaseHooks: {
    user: {
      create: {
        after: async (user) => {          // Buat/tautkan core user (maubisa_core) berdasar email (dikunci UNIQUE).
      ...
      // Saat email diverifikasi (mis. lewat OTP di /daftar atau /profil), Better Auth hanya
      // menyetel authUser.emailVerified — bukan maubisa_core.users. Sinkronkan ke core user
      // supaya badge "Terverifikasi" di /profil (yang membaca emailVerifiedAt) ikut berubah,
      // dan layanan lain yang membaca kolom itu tetap konsisten. Idempoten: hanya isi bila
      // masih null; best-effort, jangan gagalkan alur verifikasi.
      update: {
        after: async (user) => {
          if (!user.emailVerified) return;
          try {
            await prisma.user.updateMany({
              where: { email: user.email, emailVerifiedAt: null },
              data: { emailVerifiedAt: new Date(), updatedAt: new Date() },
            });
          } catch {
            /* sinkronisasi opsional; abaikan bila gagal */
          }
        },
      },
    },
