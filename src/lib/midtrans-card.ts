...
export function detectCardNetwork(raw: string): CardNetwork {
  ...
  if (/^4/.test(n)) return { id: "visa", label: "Visa", logo: "/pay/visa.png" };
  if (/^(5[1-5]|2(2[2-9]|[3-6]\d|7[01]\d|720))/.test(n))
    return { id: "mastercard", label: "Mastercard", logo: "/pay/mastercard.png" };
  if (/^3[47]/.test(n)) return { id: "amex", label: "American Express", logo: "/pay/amex.png" };
  if (/^35/.test(n)) return { id: "jcb", label: "JCB", logo: "/pay/jcb.png" };
  ...
  if (/^(6011|65|64[4-9])/.test(n)) return { id: "discover", label: "Discover", logo: "/pay/discover.png" };
  if (/^62/.test(n)) return { id: "unionpay", label: "UnionPay", logo: "/pay/unionpay.png" };
