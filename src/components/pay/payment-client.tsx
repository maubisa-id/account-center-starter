...
// Orkestrator checkout Core API (UI custom Acme). Menggantikan popup Snap: pembeli
...
export function PaymentClient({ mode, product, event, eventTitle, redirect, configured, clientKey, isProduction, eventPriceIdr, savedCards = [] }: Props) {
  ...
  // template events), untuk produk katalog pakai harga katalog. Ini yang juga ditagih.
  ...
  // Item langganan (mis. Pro) -> tampilkan pemberitahuan perpanjangan otomatis (consent).
  ...
  const submit = useCallback(async () => {
    ...
    if (mode === "guest") {
      ...
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return setMsg("Email belum valid.");
    }
    setSubmitting(true);
    try {
