...
// Yang kita rapikan adalah BINGKAI-nya (chrome bermerek Acme) supaya pembeli tak merasa
...
const PROGRAM_MARK: Partial<Record<CardNetwork["id"], { src: string; alt: string }>> = {
  visa: { src: "/pay/visa-secure.png", alt: "Verified by Visa" },
  mastercard: { src: "/pay/mc-securecode.png", alt: "Mastercard SecureCode" },
};
...
function NetworkAssurance({ network }: { network?: CardNetwork }) {
  ...
  return (
    <span className="flex items-center gap-2 opacity-80">
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src="/pay/visa-secure.png" alt="Verified by Visa" width={56} height={20} className="h-5 w-auto object-contain" />
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src="/pay/mc-securecode.png" alt="Mastercard SecureCode" width={56} height={20} className="h-5 w-auto object-contain" />
    </span>
  );
}
      ...
      >
        ...
        <div className="flex items-center justify-between gap-3 border-t border-black/[0.06] px-5 py-3">
          <span className="inline-flex items-center gap-1.5 text-[11px] font-medium text-zinc-500">
            ...
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/pay/midtrans.png" alt="Midtrans" width={80} height={14} className="h-3.5 w-auto object-contain" />
          </span>
