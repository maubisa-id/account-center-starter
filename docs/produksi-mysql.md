# Produksi MySQL — Panduan Enterprise (acme_core)

Panduan migrasi **SQLite (dev) → MySQL/Cloud SQL (produksi)** untuk `acme-account-center`,
disusun untuk standar enterprise: konsistensi engine, integritas data, performa, keamanan, dan
operasional. Berpasangan dengan `directus-acme/docs/arsitektur/acme-core-schema.sql`
...
| 3 | `.env` → `DATABASE_URL` | `file:./dev.db` | `mysql://user:pass@host:3306/acme_core` |
...
**Jalur B — DDL kanonik (paling ketat).** Jalankan `acme-core-schema.sql` yang sudah memakai
...
Pastikan DDL produksi memuat (sudah diselaraskan di `acme-core-schema.sql`):
...
- **User DB paling sempit:** akun aplikasi hanya `SELECT/INSERT/UPDATE/DELETE` pada `acme_core`.
...
- **Seed produk:** `npm run seed` atau `INSERT` dari `acme-core-schema.sql` (products).
- **Better Auth** (tabel `auth_*`) ikut ke MySQL yang sama; `DB_PROVIDER=mysql` otomatis
  mengarahkan adapter (tak perlu ubah kode).
- **SSO:** set `BETTER_AUTH_URL=https://akun.example.com` -> cookie lintas subdomain `.example.com`
...
- **Read replica** untuk laporan berat/analitik lintas Directus+core (jangan bebani primary) —
  lihat `directus-acme/docs/skalabilitas.md`.
- **Monitoring:** slow query log ON (threshold mis. 500 ms), alert pada `Threads_connected`,
  replication lag, buffer pool hit rate, disk.
- **SLO webhook** (AUDIT.md 6): p95 < 800 ms — pastikan index di atas ada supaya lookup invoice/
  subscription cepat di bawah beban.
- Satu instance Cloud SQL menampung beberapa database logis (`acme_core`, `app_acme`,
  `kelas_acme`, `thesis_acme`) — ADR-001 2.2.
...
- `directus-acme/docs/arsitektur/acme-core-schema.sql` — DDL kanonik
- `directus-acme/docs/deploy-dan-kapasitas.md` — Cloud SQL & Coolify
- `directus-acme/docs/skalabilitas.md` — read replica & penskalaan
