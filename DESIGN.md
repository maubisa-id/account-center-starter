...
### Primary
- **Acme Blue** (`#0a48b7`, `brand-500`): the single brand accent — primary buttons, links,
...
- **Teal** (`#5099bb`, `teal`) / **Lime** (`#9acc25`, `lime`): reserved accents from the brand
  palette for future data/status use. Do not scatter; the page still has one primary (Acme
  Blue).
...
- **Trust precedes the decision.** On guest checkout, a compact trust line (lock icon · badan
  hukum · otoritas terkait · Midtrans 3D Secure) sits directly under the order summary, *before* the form
  and Bayar CTA — a guest from an ad must believe before they commit. The full "Aman & tepercaya"
  block (partner logos + WhatsApp) stays below for depth. Never place all trust after the CTA.
- **Partner logos are self-hosted.** Campus/partner marks live in `/public/partners` (re-encoded to
  clean 8-bit sRGB PNG), never hotlinked from a CDN — a broken/blocked logo on a money surface
  reads as untrustworthy. Grayscale at `opacity-70`, color on hover; logo-only (no captions).
...
- **The credit card is a live, on-brand object, not a purple template.** `shared-assets/credit-card`
  ...
  `width`) but is themed with the nav gradient (`brand-500 → brand-700`), Acme wordmark, gold
